...
void UpdateGameAreas() {
    for(int area_index = 0; area_index < g_game_areas.size(); area_index++) {
        ...
        if(g_game_areas[area_index].identifier == "LevelTransition" or g_game_areas[area_index].identifier == "ShelterTransition" or g_game_areas[area_index].identifier == "HouseTransition") {
            int dest_map_index = g_game_data.current_map_index;
            Rectangle area_rect = {
                .x = g_game_areas[area_index].position.x,
                .y = g_game_areas[area_index].position.y,
                .width = (float)g_game_areas[area_index].size,
                .height = (float)g_game_areas[area_index].size
            };
            if(CheckCollisionPointRec( g_current_player->position, area_rect) ) {
                for(int level_index = 0; level_index < g_ldtk_maps.levels.size(); level_index++) {
                    ...
                    if(g_ldtk_maps.levels[level_index].identifier == g_game_areas[area_index].payload_s) {
                ...
                g_game_areas[area_index].entity_entered.EmitSignal();
            }
        }
    }
}
