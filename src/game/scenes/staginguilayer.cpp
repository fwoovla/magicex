...
StagingUILayer::StagingUILayer() {
    ...
    LoadSpriteCentered(select_chatacter_sprite, g_sprite_sheets[0], {spo.x, spo.y - 40}, 4, 24.0f, 0.10f);
...
void StagingUILayer::UpdateSelectPanel() {
    ...
    if(IsButtonHovered(select_character_left_button, g_scale)){
        ...
        if(IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
            ...
            LoadSpriteCentered(select_chatacter_sprite, g_sprite_sheets[select_index], {spo.x, spo.y - 40}, 4, 24.0f, 0.10f);
            ScaleSprite(select_chatacter_sprite, {3,3});

            LoadSpriteCentered(character_sprite, g_sprite_sheets[select_index], {cpo.x, cpo.y - 40}, 4, 24.0f, 0.10f);
            ScaleSprite(character_sprite, {3,3});
            //play_pressed.EmitSignal();
